// This script checks if cards on the allowed list are actually legal in Pioneer

import { delay } from "https://deno.land/std@0.224.0/async/delay.ts";

// Read the allowed list
...
const allowedCards = allowedList.split("\n")
  .map((line: string) => line.replace(/"/g, "").trim())
  .filter(Boolean); // Remove empty lines

console.log(`Total allowed cards: ${allowedCards.length}`);
...
console.log("Checking Pioneer legality for allowed cards...");
...
for (let i = 0; i < allowedCards.length; i += batchSize) {
  try {
    ...
    if (i + batchSize < allowedCards.length) {
      await delay(delayBetweenBatches);
    }
  } catch (error) {
    console.error(`Error processing batch starting at index ${i}:`, error);
    // Add the whole batch to failed cards
    failedCards.push(...allowedCards.slice(i, i + batchSize));
  }
}
...
// Filter out cards that are already Pioneer legal
const redundantAllowedCards = results.filter((card) => card.isPioneerLegal);

console.log(
  "\nCards on the allowed list that are already Pioneer legal (redundant to allow):",
);
redundantAllowedCards.forEach((card) => {
  console.log(`- ${card.name}`);
});
console.log(`\nTotal redundant allowed cards: ${redundantAllowedCards.length}`);

// Write results to a file
const resultText =
  `# Cards on the allowed list that are already Pioneer legal (redundant to allow)
${redundantAllowedCards.map((card) => card.name).join("\n")}

Total: ${redundantAllowedCards.length} cards`;

await Deno.writeTextFile("redundant-allowed-cards.txt", resultText);
console.log("\nResults written to redundant-allowed-cards.txt");
